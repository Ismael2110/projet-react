        ...
        const fetchData = async () => {
            try {
                ...
                const newAnswerArray = response.Questions.map(
                    (question, index) => {
                        if (
                            question.question === "quel département souhaitez-vous rejoindre ?"
                        ) {
                            return {
                ...
                setQuestionFilter(response.Questions.filter((item)=>{
                    return (item.question !== "quel département souhaitez-vous rejoindre ?")
                }))
                    ...
                    <View style={{justifyContent : "center", alignItems : "center"}}>
                        ...
                        <View style={styles.questionContainer}>
                            {QuestionFilter.map((question, index) => {
                                if (
                                    question.question ===
                                    "quel département souhaitez-vous rejoindre ?"
                                )
                                    return;
                                // setCount(count + 1)
                                return (
                            ...
                            <View style={styles.questionSmallContainer}>
                                <Text style={styles.questionTitle}>
                                    {data.Questions.length}. quel département souhaitez-vous réjoindre ?
                                </Text>
                                <RNPickerSelect
                                    onValueChange={(value) => {
